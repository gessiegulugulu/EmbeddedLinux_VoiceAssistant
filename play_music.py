...
def play_mp3(songname):
    path=os.getcwd()+"\\music\\"+songname
    os.system('sox '+path+'.wav -t waveaudio')
    ...
    def download(self,song):
        ...
        if download_url:
            try:
                ...
                urllib.request.urlretrieve(self.download_url+download_url, './music/'+songname+ '.m4a')
                print('Successfully Download:' + song['singer']+'--'+song['song_name'] + '.m4a')
                os.system('ffmpeg -i ' +'./music/'+songname+'.m4a ' +'./music/'+songname+'.wav')
                os.remove('./music/'+songname+'.m4a ')
                return songname
            except:
